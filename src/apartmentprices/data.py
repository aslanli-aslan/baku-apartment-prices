...
def calculate_metro_features(df):
    ...
    a = ( np.sin(dlat / 2) ** 2 + np.cos(lat[:, None]) * np.cos(metro_lat[None, :]) * np.sin(dlng / 2) ** 2 )

    distances = 2 * R * np.arcsin(np.sqrt(a))

    result = df.copy()
    result["nearest_metro_distance_km"] = distances.min(axis=1)
    result["nearest_metro_id"] = pd.Series(distances.argmin(axis=1), index=df.index, dtype="category")
